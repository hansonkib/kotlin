...
private fun Project.localRepoDir() = File("${project.rootDir.absoluteFile}/dependencies/repo")
...
fun RepositoryHandler.localRepo(project: Project): IvyArtifactRepository = ivy {
    val baseDir = project.localRepoDir()
    setUrl(baseDir)
...
object IntellijRootUtils {
    fun getRepositoryRootDir(project: Project): File = with(project.rootProject) {
        return File(localRepoDir(), "kotlin.build")
    }
...
fun Project.intellijUltimateRootDir() =
    if (isIntellijUltimateSdkAvailable())
        File(localRepoDir(), "kotlin.build/ideaIU/${rootProject.extra["versions.intellijSdk"]}/artifacts")
    else
